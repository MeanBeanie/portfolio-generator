...
struct website parse(struct token* tokens, int token_count, int page_count){
	...
	for(int i = 0; i < token_count; i++){
		switch(tokens[i].type){
			...
			case TT_SECTION:
			{
				pagenr++;
				text_element_cap = 8;
				cur_page.title = (string){ .start = tokens[i].start+1, .len = tokens[i].len-1 };
			...
			case TT_LINK:
			{
				if(i+1 >= token_count || (i+1 < token_count && tokens[i+1].type != TT_SECTION_REF)){
			...
			case TT_SECTION_REF: // its handled in the TT_LINK
	...
	for(int i = 0; i < page_count; i++){
		if(strncmp("index", res.pages[i].title.start, 5) == 0){
			passed = 1;
		}
	}
	if(passed == 0){
